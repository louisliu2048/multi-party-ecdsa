...
mod bench {
    ...
    > {
        ...
        for i in s.clone() {
            ...
            let data = fs::read_to_string(proj_dir.clone().join(file_path).to_str().unwrap())
    ...
    >) {
        ...
        let sign_keys_vec = (0..ttag)
            .map(|i| SignKeys::create(&private_vec[usize::from(s[i])], &vss_scheme_vec[i][usize::from(s[i])], s[i], &s))
    ...
    targets = self::bench_full_sign_party_one_three_serial_raw}
}
